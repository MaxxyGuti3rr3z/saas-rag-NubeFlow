...
🔗 **Demo en vivo:** *(pegar aquí el link de tu app en Streamlit Cloud)*
...
- 💬 Interfaz conversacional desarrollada con Streamlit, con historial de conversación.
...
Pregunta del usuario
      │
      ▼
Búsqueda semántica (similarity_search, top-k)
      │
      ▼
...
Modelo Groq (Llama 3.3 70B) — genera la respuesta restringida al contexto
      │
      ▼
Interfaz Streamlit — respuesta + fuentes citadas + historial
```
...
> ejecución (en `/tmp` cuando corre en Streamlit Cloud) a partir de los documentos en `data/`.
...
> (produce el error `attempt to write a readonly database`), por lo que la base vectorial se
> persiste en `/tmp` en tiempo de ejecución en lugar de en una carpeta del repositorio.
 
📸 **Evidencia del deploy:** *(agregar aquí captura de pantalla o video corto de la app
corriendo en Streamlit Cloud, como pide el desafío)*
...
| "¿Cuál es la política de mascotas en la oficina?" *(no existe en la base)* | El agente responde que no encontró la información, en vez de inventarla |
...
2. Solo envía al modelo el contexto recuperado (no usa conocimiento externo).
3. El prompt instruye explícitamente a admitir cuando la información no está disponible.
4. Cuando la búsqueda no devuelve resultados, responde con un mensaje de fallback en vez
   de generar una respuesta con el LLM.
---
